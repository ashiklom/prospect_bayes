...
# Calculate summary statistics
    mu <- colMeans(samples.sub)
    names(mu) <- sprintf("%s.mu", parnames)
    sigma <- apply(samples.sub, 2, sd)
    names(sigma) <- sprintf("%s.sigma", parnames)
    q25 <- apply(samples.sub, 2, quantile, 0.025)
    names(q25) <- sprintf("%s.q25", parnames)
    med <- apply(samples.sub, 2, median)
    names(med) <- sprintf("%s.med", parnames)
    q975 <- apply(samples.sub, 2, quantile, 0.975)
